...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataset):
        ...
        self.margins, self.freq = _load_margin(config.cache_root, name)
        ...
        for i in range(len(self.entries)):
            if len(self.entries[i]['answer']['labels']) == 0:
                continue
            y = self.entries[i]['answer']['labels'][0]
            if y not in self.cls_num_list:
                self.cls_num_list[y] = 0
            self.cls_num_list[y] += 1
        self.cls_num_list = [self.cls_num_list[i] if i in self.cls_num_list else 1 for i in range(self.num_ans_candidates)]
    ...
    def __getitem__(self, index):
        ...
        return features, question, target, target_margin, bias, question_id, freq_margin0, q_type, ans_type
